...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8079"]
            APIMAPA["SERVIDOR DE APLICACIONES<br/>contenedor api-mapa<br/>hostname: api-mapa · escucha en 8031"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8031"| APIMAPA
    NAV -->|"localhost:8079"| FRONTFLASK
    APIMAPA -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    FRONTFLASK -->|"http://api-mapa:8031<br/>al controlador, por el NOMBRE"| APIMAPA
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15460"| POSTGRES
```
...
```powershell
docker network inspect proyecto_paradigmas_mapa_conocimiento1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas_mapa_conocimiento1` produce
la red `proyecto_paradigmas_mapa_conocimiento1_default`. Tres formas de confirmarlo:
...
docker inspect paradigmas-mapa-front --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_paradigmas_mapa_conocimiento1_default has active endpoints (name:"paradigmas-mapa-front" …)
```
